...
class CookiesJwtAuthentication(JWTAuthentication):
    def authenticate(self, request):
        ...
        try:
            ...
            print(
                f"Auth: Access token failed validation/user lookup (TokenError: {e}). Attempting refresh..."
            )
            ...
            if refresh_token:
                try:
                    ...
                    print(
                        f"Auth: User '{user.username}' authenticated with refreshed token."
                    )
                    return (user, validated_token)
                    ...
                    print(
                        f"Auth: Refresh token also failed (TokenError: {refresh_e}). Raising AuthenticationFailed."
                    )
